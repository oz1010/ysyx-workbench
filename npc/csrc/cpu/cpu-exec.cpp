...
static void trace_and_difftest(Decode *_this, vaddr_t dnpc) {
  ...
  if (npc_ctx.state==NPC_RUNNING && scan_point(POINT_WATCH)) npc_ctx.state = NPC_STOP;
#endif
#ifdef CONFIG_BREAKPOINT
  if (npc_ctx.state==NPC_RUNNING && scan_point(POINT_BREAK)) npc_ctx.state = NPC_STOP;
#endif
  if (g_print_step) { IFDEF(CONFIG_ITRACE, puts(_this->logbuf)); }
  IFDEF(CONFIG_DIFFTEST, difftest_step(_this->pc, dnpc));

//   IRINGBUF_UPDATE(MUXDEF(CONFIG_ISA_x86, _this->snpc, _this->pc), _this->logbuf, npc_ctx.state == NPC_RUNNING);
}
