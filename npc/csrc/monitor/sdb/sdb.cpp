...
{
    static const char *str_wp = MUXDEF(CONFIG_WATCHPOINT, "/w", "");
    static const char *str_bp = MUXDEF(CONFIG_BREAKPOINT, "/b", "");
    if (args)
    {
        if (strcmp(args, "r") == 0)
            isa_reg_display();
        else if (ISDEF(CONFIG_WATCHPOINT) && strcmp(args, "w") == 0)
