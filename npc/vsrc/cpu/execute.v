    ...
    if (rst) begin
        ...
        if (w_ex_prev_state == `SCOM_FOUND) begin
            ...
            case (inst_code)
                ...
                `INST_BEQ:          if (src1==src2) r_dnpc <= pc + imm;
                `INST_BNE:          if (src1!=src2) r_dnpc <= pc + imm;
                `INST_BLT:          if (s_src1<s_src2) r_dnpc <= pc + imm;
                `INST_BGE:          if (s_src1>=s_src2) r_dnpc <= pc + imm;
                `INST_BLTU:         if (src1< src2) r_dnpc <= pc + imm;
                `INST_BGEU:         if (src1>=src2) r_dnpc <= pc + imm;
                ...
                `INST_ECALL:        r_dnpc <= dpi_raise_intr((0<<31|11<<0), pc); // abstract-machine/am/src/riscv/nemu/cte.c约定 GPR1 is event when mcause is Environment call from M-mode (0<<31 | 11<<0)
                ...
                `INST_MRET:         r_dnpc <= dpi_query_intr();
