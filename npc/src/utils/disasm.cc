...
EXTERN_C void init_disasm(const char *triple)
{
    ...
    gIP = target->createMCInstPrinter(llvm::Triple(gTriple), AsmInfo->getAssemblerDialect(),
                                      *AsmInfo, *gMII, *gMRI);
...
EXTERN_C void disassemble(char *str, int size, uint64_t pc, uint8_t *code, int nbyte)
{
    MCInst inst;
