...
#if 0
#define TRACE(VAL)                                                                \
    printf("Enter %s, token|%d %s val|%d success|%d\n", __FUNCTION__, *token_idx, \
           *token_idx < nr_token ? tokens[*token_idx].str : "NONE", VAL, *success)
#else
#define TRACE(VAL)
#endif
#define TOKEN_EXCEPT()                                                        \
    do                                                                        \
    {                                                                         \
        Log("token except at %d|%d %s.", *token_idx, tokens[*token_idx].type, \
...
bool check_parentheses(int begin, int end, bool *success)
{
    ...
    if (lparenth != rparenth)
    {
        Log("miss match parenteses at %d|'%s' %d|'%s'.", begin, tokens[begin].str, end,
            tokens[end].str);
        *success = false;
        return 0;
    }
...
int find_low_op_pos(int begin, int end, bool *success)
{
    static op_prio_t op_prios[] = {
        /* type   priority */
        {'*', 6}, {'/', 6}, {'+', 7}, {'-', 7}, {TK_EQ, 10}, {TK_NEQ, 10}, {TK_AND, 14},
    };
    int pos = -1;
    uint32_t priority = 0;
    ...
    if (lparenth != rparenth)
    {
        Log("find op miss match parenteses at %d|'%s' %d|'%s'.", begin, tokens[begin].str, end,
            tokens[end].str);
        *success = false;
        return 0;
    }
...
word_t eval(int begin, int end, bool *success)
{
    ...
    else
    {
        ...
        {
            *success = false;
            Log("miss low operator pos at %d|'%s' %d|'%s'.", begin, tokens[begin].str, end,
                tokens[end].str);
            return 0;
        }
        CHECK_SUCCESS();
        ...
        switch (tokens[op_pos].type)
        {
            ...
            case '/':
                ...
                break;
            default:
                Log("unknown op error at %d|'%s' %d|'%s'.", begin, tokens[begin].str, end,
                    tokens[end].str);
                *success = false;
                return 0;
        }
