...
int dtm_init(cpu_opt_t *cpu_opt)
{
#if CONFIG_DEBUG_MODULE
    dm_add_def_cpu_opt(cpu_opt);
    ...
    for (int i=0; i<num_fields; ++i)
    {
        ...
        if (ir_scan) {
            ...
            if (ctx->ir == dtm_ri_dmi)
            {
                ...
                val_bytes = DIV_ROUND_UP(num_bits, 8);
                scan_idcode_data = (uint8_t *)malloc(val_bytes);
...
void dtm_update(dm_exec_inst_period_t period, uint32_t inst, CPU_state *c)
{
    dtm_msg_t recv_msg;
    int handle_ret = 0;
    
    // 根据情况切换到调试状态，比如ebreak或breakpoint
    dmi_prepare_status(period, inst);
