...
// #define _DM_RI_STR_CASE(N) [dm_ri_##N] = #N
...
static cpu_opt_t *def_cpu_opt = NULL;
...
{
#undef _DM_RI_STR_CASE
#define _DM_RI_STR_CASE(N) case dm_ri_##N: return #N; break
    switch (idx) {
        _DM_RI_STR_CASE(data0);
        _DM_RI_STR_CASE(data1);
        _DM_RI_STR_CASE(data2);
        _DM_RI_STR_CASE(data3);
        _DM_RI_STR_CASE(data4);
        _DM_RI_STR_CASE(data5);
        _DM_RI_STR_CASE(data6);
        _DM_RI_STR_CASE(data7);
        _DM_RI_STR_CASE(data8);
        _DM_RI_STR_CASE(data9);
        _DM_RI_STR_CASE(data10);
        _DM_RI_STR_CASE(data11);
        _DM_RI_STR_CASE(dmcontrol);
        _DM_RI_STR_CASE(dmstatus);
        _DM_RI_STR_CASE(hartinfo);
        _DM_RI_STR_CASE(haltsum1);
        _DM_RI_STR_CASE(hawindowsel);
        _DM_RI_STR_CASE(hawindow);
        _DM_RI_STR_CASE(abstractcs);
        _DM_RI_STR_CASE(command);
        _DM_RI_STR_CASE(abstractauto);
        _DM_RI_STR_CASE(confstrptr0);
        _DM_RI_STR_CASE(confstrptr1);
        _DM_RI_STR_CASE(confstrptr2);
        _DM_RI_STR_CASE(confstrptr3);
        _DM_RI_STR_CASE(nextdm);
        _DM_RI_STR_CASE(progbuf0);
        _DM_RI_STR_CASE(progbuf1);
        _DM_RI_STR_CASE(progbuf2);
        _DM_RI_STR_CASE(progbuf3);
        _DM_RI_STR_CASE(progbuf4);
        _DM_RI_STR_CASE(progbuf5);
        _DM_RI_STR_CASE(progbuf6);
        _DM_RI_STR_CASE(progbuf7);
        _DM_RI_STR_CASE(progbuf8);
        _DM_RI_STR_CASE(progbuf9);
        _DM_RI_STR_CASE(progbuf10);
        _DM_RI_STR_CASE(progbuf11);
        _DM_RI_STR_CASE(progbuf12);
        _DM_RI_STR_CASE(progbuf13);
        _DM_RI_STR_CASE(progbuf14);
        _DM_RI_STR_CASE(progbuf15);
        _DM_RI_STR_CASE(authdata);
        _DM_RI_STR_CASE(haltsum2);
        _DM_RI_STR_CASE(haltsum3);
        _DM_RI_STR_CASE(sbaddress3);
        _DM_RI_STR_CASE(sbcs);
        _DM_RI_STR_CASE(sbaddress0);
        _DM_RI_STR_CASE(sbaddress1);
        _DM_RI_STR_CASE(sbaddress2);
        _DM_RI_STR_CASE(sbdata0);
        _DM_RI_STR_CASE(sbdata1);
        _DM_RI_STR_CASE(sbdata2);
        _DM_RI_STR_CASE(sbdata3);
        _DM_RI_STR_CASE(haltsum0);
    }
#undef _DM_RI_STR_CASE
...
{
    word_t *reg = cur_dm_ctx->cpu_opt->get_gpr(cur_cpu, reg_idx);
    if (write)
        *reg = *reg_value;
    else
        *reg_value = *reg;
...
static int dm_access_cpu_memory(dm_reg_command_t *cmd, uint32_t size, uint32_t *arg0, uint32_t *arg1, uint32_t *arg2)
{
    ...
    for (int i=0; i<size; i+=access_len) {
        ...
        if (addr >= PMEM_LEFT && addr <= PMEM_RIGHT) {
            ctx->cpu_opt->access_mem(write, addr, access_len, (uint8_t *)arg0);
            // if (write) {
            //     ctx->cpu_opt->vaddr_write(addr, access_len, *arg0);
            // } else {
            //     word_t mem_val = ctx->cpu_opt->vaddr_read(addr, access_len);
...
int dm_init(void)
{
    ...
    for (size_t i=0; i<DM_ARRAY_SIZE(dm_ctx_list); ++i)
    {
        ...
        ctx->handle_reg_funcs[dm_ri_dmcontrol] = dm_handle_dmcontrol;
        ctx->handle_reg_funcs[dm_ri_abstractcs] = dm_handle_abstractcs;
        ctx->handle_reg_funcs[dm_ri_command] = dm_handle_command;
    }
