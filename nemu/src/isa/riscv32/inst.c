...
#define ERROR_INV_OPN(_PC) NEMUTRAP((_PC), (-2))
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 00000 11", lb     , I, R(rd) = SEXT(Mr(src1 + SEXT(imm, 12), 1), 8) );
  INSTPAT("??????? ????? ????? 001 ????? 00000 11", lh     , I, R(rd) = SEXT(Mr(src1 + SEXT(imm, 12), 2), 16) );
  INSTPAT("??????? ????? ????? 010 ????? 00000 11", lw     , I, R(rd) = SEXT(Mr(src1 + SEXT(imm, 12), 4), 32) );
  INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu    , I, R(rd) = Mr(src1 + SEXT(imm, 12), 1)&0xFF );
  INSTPAT("??????? ????? ????? 101 ????? 00000 11", lhu    , I, R(rd) = Mr(src1 + SEXT(imm, 12), 2)&0xFFFF );
  INSTPAT("??????? ????? ????? 000 ????? 01000 11", sb     , S, Mw(src1 + SEXT(imm, 12), 1, src2&0xFF) );
  INSTPAT("??????? ????? ????? 001 ????? 01000 11", sh     , S, Mw(src1 + SEXT(imm, 12), 2, src2&0xFFFF) );
  INSTPAT("??????? ????? ????? 010 ????? 01000 11", sw     , S, Mw(src1 + SEXT(imm, 12), 4, src2) );
  INSTPAT("??????? ????? ????? 000 ????? 00100 11", addi   , I, R(rd) = (int32_t)src1 + (int32_t)SEXT(imm, 12) );
  INSTPAT("??????? ????? ????? 010 ????? 00100 11", slti   , I, R(rd) = (int32_t)src1 < (int32_t)SEXT(imm, 12) );
  ...
  INSTPAT("0000000 ????? ????? 001 ????? 00100 11", slli   , I, if(imm & 0x20) ERROR_INV_OPN(s->pc); R(rd) = src1 << (imm & 0x1F) );
  INSTPAT("0000000 ????? ????? 101 ????? 00100 11", srli   , I, if(imm & 0x20) ERROR_INV_OPN(s->pc); R(rd) = src1 >> (imm & 0x1F) );
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, if(imm & 0x20) ERROR_INV_OPN(s->pc); R(rd) = (src1 >> (imm & 0x1F) | ( ((src1&0x80000000)?0xffffffff:0) << (32-(imm & 0x1F))) ) );
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 01100 11", sra    , R, R(rd) = src1 >> (src2 & 0x1F); if (src1 & 0x80000000) R(rd) |= ~BITMASK(32-(src2 & 0x1F)) );
  INSTPAT("0000000 ????? ????? 110 ????? 01100 11", or     , R, R(rd) = src1 | src2 );
  INSTPAT("0000000 ????? ????? 111 ????? 01100 11", and    , R, R(rd) = src1 & src2 );
  // INSTPAT("??????? ????? ????? 000 ????? 00100 11", fence  , I, xxx );
  // INSTPAT("??????? ????? ????? 000 ????? 00100 11", fence.i, I, xxx );
  INSTPAT("0000000 00000 00000 000 00000 11100 11", ecall  , I, s->dnpc = RAISE_INTR((0<<31|11<<0), s->pc) ); // abstract-machine/am/src/riscv/nemu/cte.c约定 GPR1 is event when mcause is Environment call from M-mode (0<<31 | 11<<0)
  ...
  INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh   , R, R(rd) = (uint64_t)(SEXT(src1, 32) * SEXT(src2, 32)) >> 32 );
  INSTPAT("0000001 ????? ????? 010 ????? 01100 11", mulhsu , R, R(rd) = (uint64_t)(SEXT(src1, 32) * src2) >> 32 );
  INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu  , R, R(rd) = (uint64_t)((uint64_t)src1 * (uint64_t)src2) >> 32 );
  INSTPAT("0000001 ????? ????? 100 ????? 01100 11", div    , R, R(rd) = (int32_t)SEXT(src1, 32) / (int32_t)SEXT(src2, 32) );
  INSTPAT("0000001 ????? ????? 101 ????? 01100 11", divu   , R, R(rd) = src1 / src2 );
  INSTPAT("0000001 ????? ????? 110 ????? 01100 11", rem    , R, R(rd) = (int32_t)SEXT(src1, 32) % (int32_t)SEXT(src2, 32) );
